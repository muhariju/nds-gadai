...
public interface PelangganRepo extends JpaRepository<PelangganEntity, String>, JpaSpecificationExecutor<PelangganEntity> {
        @Query(value = "SELECT COUNT(*) FROM ms_pelanggan WHERE LOWER(id) = LOWER(:id)", nativeQuery = true)
        long countById(@Param("id") String id);

        @Query(value = "SELECT COUNT(*) FROM ms_pelanggan WHERE nomor_hp = :nomorHp ", nativeQuery = true)
        long countByNomorHP(@Param("nomorHp") String nomorHp);

        @Query(value = "SELECT COUNT(*) FROM ms_pelanggan WHERE nomor_ktp = :nomorKtp ", nativeQuery = true)
        long countByNomorKTP(@Param("nomorKtp") String nomorKtp);

        @Modifying
        @Query(value = "UPDATE ms_pelanggan SET rec_status = '"
